...
int main(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "s:w:m:l:c:t:v:")) != -1)
  {
    ...
    {
    case 's':
      seed = read_arg_int(opt);
      break;
    case 'w':
      win_size = read_arg_int(opt);
      break;
    case 'm':
      nsimmax = read_arg_int(opt);
      break;
    case 'l':
      lossprob = read_arg_float(opt);
      break;
    case 'c':
      corruptprob = read_arg_float(opt);
      break;
    case 't':
      if ((lambda = atof(optarg)) <= 0.0)
      {
        fprintf(stderr, "Invalid value for -%c\n", opt);
        exit(-1);
      }
